  ...
  app.get("/api/getnum", async (req, res) => {
    try {
      ...
      if (data && data.meta && data.meta.code === 200 && data.data) {
        const payload = {
          number: data.data.full_number || data.data.no_plus_number || data.data.national_number,
          country: data.data.country || "Unknown",
          id: data.rid || `session-${Date.now()}`
        };
        res.json(payload);
      } else {
        const errMsg = (data && data.message) || (data && data.meta && data.meta.msg) || "No virtual number available at the moment. Try again.";
        res.status(400).json({ 
          error: errMsg, 
          details: data 
        });
